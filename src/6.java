...
class Solution {
    public String convert(String s, int numRows) {
        ...
        Map<Integer, ArrayList<Character>> tempRes = new HashMap<>();
        for (int j = 0; j < numRows; j++) {
            tempRes.put(j, new ArrayList<>());
        }

        while (i < s.length()) {
            int index = i % (numRows * 2 - 2);
            if (index < numRows) {
                tempRes.get(index).add(s.charAt(i));
            } else {
                tempRes.get((numRows - 1) * 2 - index).add(s.charAt(i));
            }
            i++;
        }
        ...
        for (int j = 0; j < numRows; j++) {
            List<Character> row = tempRes.get(j);
            for (int m = 0; m < row.size(); m++) {
                builder.append(row.get(m));
            }
        }
